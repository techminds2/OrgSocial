...
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    if (!Number.isFinite(addUserId)) return NextResponse.json({ error: "Bad userId" }, { status: 400 });
    if (!VALID_ROLES.includes(role)) return NextResponse.json({ error: "Bad role" }, { status: 400 });
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  try {
    ...
    if (!Number.isFinite(targetUserId)) return NextResponse.json({ error: "Bad userId" }, { status: 400 });
    if (!VALID_ROLES.includes(role)) return NextResponse.json({ error: "Bad role" }, { status: 400 });

    // optional safety: don't let admin remove own admin
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  try {
    ...
    // optional: don't remove yourself
