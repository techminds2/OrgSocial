...
export async function GET() {
  try {
    ...
    const departmentManagers = corporateUsers.filter(
    ...
    const reports = await prisma.regionalDailyReport.findMany({
      where: {
        reportYmd,
        authorId: {
          in: departmentManagers.map((u) => Number(u.id)),
        },
      },
      orderBy: {
        updatedAt: "desc",
    ...
    const items = departmentManagers.map((u) => {
    ...
    console.error("REGIONAL TODAY STATUS ERROR:", e);
